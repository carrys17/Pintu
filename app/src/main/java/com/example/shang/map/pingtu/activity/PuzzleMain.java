...
import com.example.shang.map.pingtu.adapter.GridItemsAdapter;
import com.example.shang.map.pingtu.bean.ItemBean;
import com.example.shang.map.pingtu.utils.GameUtil;
import com.example.shang.map.pingtu.utils.ImagesUtil;
...
//            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mResId != 0) { //那边传过来时，当选到最后的自定义的时候，没传picSelectedID（传的是mPicPath），所以为0时就是自定义的
    ...
    private void initViews() {
      ...
      //  mGvPuzzleMainDetail.setFastScrollEnabled(false);// 禁止上下滑动失败，两个思路，一个自定义view继承GridView,一个换成GridLayout
        ...
        mGvPuzzleMainDetail.setHorizontalSpacing(20);  // 莫名其妙在这里解决了那个水平切割少一点点的问题
